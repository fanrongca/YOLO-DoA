...
#   Description : Data preprocessing, model training, trained model storage function
#
# ================================================================
...
valid_num = 42000
test_num = 192600
angel_range = 90  # the range of angels according to the requirements
...
def GetProcessedData(data_batch, is_test=False):
    ...
    Label_Merge -- the labels for post process
    """
    Data_String = data_batch['IQData']
    Data_Merge = []
    ...
    true_boxes = np.zeros((BATCH_SIZE, signal_num, 4), dtype=np.float32)

    train_output_sizes = np.asarray(train_output_sizes, dtype=np.int32)

    for i in range(BATCH_SIZE):
        label_box[i], true_boxes[i] = tools.preprocess_true_boxes(gbboxes_iou[i], train_output_sizes, strides)
...
def main(_):
    ...
    tf_true_boxes = tf.placeholder(dtype=tf.float32, shape=[BATCH_SIZE, signal_num, 4], name='boxes')
    trainable = tf.placeholder(dtype=tf.bool, name='training')
    yolo_model = YOLODOA(tf_input_data, trainable)
    giou_loss, conf_loss = yolo_model.loss_calculation(
        tf_label_box,
        tf_true_boxes)
            ...
            for epoch in range(TRAINING_EPOCH):
                ...
                for i in range(TRAINING_STEPS):
                    ...
                    if step % 20 == 19:
                        ...
                        postprocess_predictedbox(Train_Valid_Dir, pred_box_value, valid_csv_name, labels, true_csv_name)
                        ...
                        valid_csv_name = str(step) + '_valid.csv'
                        true_csv_name = str(step) + '_true.csv'
                        postprocess_predictedbox(Test_Dir, pred_box_value, valid_csv_name, labels, true_csv_name)
                ...
                if epoch > 0 and epoch % 2 == 1:
                    ...
                    inputs_params = {'tf_input_data': tf.saved_model.utils.build_tensor_info(tf_input_data),
                                     ...
                                     'is_training': tf.saved_model.utils.build_tensor_info(trainable)
                                     }
...
def postprocess_predictedbox(Save_Dir, predicted_boxes, csv_name, labels, true_csv_name):
    """
    Implementation for post process of YOLO-DoA
    Arguments:
    Save_Dir -- file dir for saving the predicted boxes during the training
    predicted_boxes -- the 72 predicted boxes need to eliminate the redundant boxes
    ...
    result = []
    for i in range(BATCH_SIZE):
        pred_bbox = np.reshape(predicted_boxes[i], (-1, 5))
        bboxes = tools.postprocess_boxes(pred_bbox, output_shape, 0.1)
        bboxes = tools.soft_nms(bboxes)
